import axios from "axios";

// Type
import type { Schedule, UserAgenda, UserData, UserSchedule } from "../types/type";

const url = "http://localhost:3000";

export const getGenerateSchedules = async () => {
  try {
    const { data } = await axios.get(`${url}/agenda/`);
    const schedules: UserData[] = data.schedule;

    return schedules;
  } catch (err: unknown) {
    console.error("Erro ao obter agenda do usuário:", err);

    return null;
  }
};

export const getAllSchedules = async () => {
  try {
    const res = await axios.get(`${url}/agenda/todos`);
    return res.data;
  } catch (err: unknown) {
    console.error("Erro ao obter agenda do usuário:", err);

    return null;
  }
};

export const userAgenda = async (email: string): Promise<Schedule[]> => {
  try {
    const { data } = await axios.get(`${url}/agenda/${email}`, {
      withCredentials: true,
    });

    const filteredData: Schedule[] = data.map((item: UserSchedule) => ({
      id: item._id,
      date: item.date,
      time: item.time,
    }));

    return filteredData;
  } catch (err: unknown) {
    console.error("Erro ao obter agenda do usuário:", err);
    if (axios.isAxiosError(err)) {
      return err?.response?.data;
    }
    return [];
  }
};

export const createUserSchedule = async (user: UserAgenda) => {
  try {
    const { data } = await axios.post(`${url}/agenda/scheduleTime`, user, {
      withCredentials: true,
    });

    return data;
  } catch (err: unknown) {
    console.error("Erro ao marcar horário:", err);
    if (axios.isAxiosError(err)) {
      return err?.response?.data;
    }
    return null;
  }
};

export const deleteUserSchedule = async (id: string) => {
  console.log(id);

  try {
    const res = await axios.delete(`${url}/agenda/scheduleTime/${id}`, {
      withCredentials: true,
    });

    return res.data;
  } catch (err: unknown) {
    console.error("Erro ao cancelar horário:", err);
    // Type guard to check if err is an Axios error
    if (axios.isAxiosError(err)) {
      return err?.response?.data;
    }
    return null;
  }
};
